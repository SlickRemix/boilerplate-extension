...
class Core_Functions {
	 ...
	 * @since 1.0.0
	 */
	public function ft_gallery_tab_notice_html() {
		echo '<div class="ft-gallery-notice"></div>';
	}


	/**
	 * FT Gallery CPT Request Redirect Fix
	 *
	...
	public function ft_gallery_locate_template( $located ) {
		...
		if ( true == $use_template ) {
			...
			} elseif ( file_exists( trailingslashit( CURRENT_PLUGIN_FOLDER_DIR . 'templates/' . $template_name ) ) ) {
				$located = trailingslashit( CURRENT_PLUGIN_FOLDER_DIR . 'templates/' . $template_name );
			}
			// Use Plugins Album template.
			if ( empty( $located ) ) {

				$plugin_location = CURRENT_PLUGIN_FOLDER_DIR . 'templates/' . $template_name;
